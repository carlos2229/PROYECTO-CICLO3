...
CREATE TABLE IF NOT EXISTS `Softgeb`.`Softgeb` (
  ...
  CONSTRAINT `fk_Equipos Biomedicos_ubicacion`
    FOREIGN KEY (`ubicacion_id_ubicacion`)
    REFERENCES `Softgeb`.`ubicacion` (`id_ubicacion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_equipos biomedicos_estatus1`
    FOREIGN KEY (`estatus_id_estatus`)
    REFERENCES `Softgeb`.`estatus` (`id_estatus`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
